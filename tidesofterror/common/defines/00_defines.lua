...
NGame = {
	START_DATE = "1868.1.1.12",
	END_DATE = "1874.1.1.1",
	MAP_SCALE_PIXEL_TO_KM = 0.004,					-- Orignally 7.114 with global earth. britain map is 0.004. divide any normal globe value by 1778.5 to get britain scale
	...
	MISSION_REMOVE_FROM_INTERFACE_DEFAULT = 0,		-- Default days before a mission is removed from the interface after having failed or completed
	...
	MUSIC_PLAYER_RECENTLY_PLAYED_SIZE = 5,			-- The music player keeps track of recently played music to try and avoid playing the same songs too often. This determines the max number of songs in the recently played list.
},
...
NDiplomacy = {
	...
	WARGOAL_WORLD_TENSION_REDUCTION = 0.0,			-- Reduction of pp cost for wargoal at 100% world tension, scales linearly
	WARGOAL_JUSTIFY_TENSION_FROM_PRODUCTION = 0.0,	-- Base value scaled by production capacity of country compared to biggest country
	...
	TENSION_DECAY_DAILY = 0.0,					-- Each day tension decays this much for Threat sources which are no longer relevant. Replaces TENSION_DECAY as of 1.12.0
	TENSION_SIZE_FACTOR = 1.0,						-- All action tension values are multiplied by this value
	TENSION_TIME_SCALE_START_DATE = "1868.1.1.12",	-- Starting at this date, the tension values will be scaled down (will be equal to 1 before that)
	TENSION_TIME_SCALE_MONTHLY_FACTOR = 0.0,		-- Timed tension scale will be modified by this amount starting with TENSION_TIME_SCALE_START_DATE
...
NCountry = {
	EVENT_PROCESS_OFFSET = 4,						-- Events are checked every X day per country or state (1 is ideal, but CPU heavy)
	...
	DEFAULT_WAR_SUPPORT = 1.0,						-- Default war support if not scripted otherwise.
	...
	MIN_MAJOR_COUNTRIES	= 3,						-- MIN_MAJOR_COUNTRIES countries with most factories will be considered as major countries
...
NResistance = {
	...
	RESISTANCE_POP_LOW_CUTOFF = 6000,
	RESISTANCE_POP_VERY_LOW_CUTOFF = 2800,
	...
	RESISTANCE_DECAY_MODIFIER_FACTORS = {  -- resistance decay modifier when resistance hits a certain percentage
		10, 0, -- below 10% it has a -50% modifier on decay
		20, 0, -- below 20% it has a -25% modifier on decay
	},
	...
	STATE_COMPLIANCE_DECAY_FOR_LOST_STATES = 0.15, -- daily compliance decay for the states you lost control of

},

NProduction = {
	MAX_EQUIPMENT_RESOURCES_NEED = 7, 	-- Max number of different strategic resources an equipment can be dependent on.
	...
	BASE_FACTORY_START_EFFICIENCY_FACTOR = 100,	-- Base start efficiency for factories expressed in %.
	BASE_FACTORY_MAX_EFFICIENCY_FACTOR = 100,	-- Base max efficiency for factories expressed in %.
	BASE_FACTORY_EFFICIENCY_GAIN = 0,		-- Base efficiency factor.
	...
	ANNEX_FUEL_RATIO = 0.0,	-- How much fuel will be transferred on annexation
	ANNEX_CONVOYS_RATIO = 0.15,			-- How many convoys will be transferred on annexation
	MIN_POSSIBLE_TRAINING_MANPOWER = 1000,	-- How many deployment lines minimum can be training
	...
	CAPITULATE_FUEL_RATIO = 0.0, -- How much fuel will be transferred on capitulation
...
NBuildings = {
	...
	MAX_BUILDING_LEVELS = 25,			-- Max levels a building can have.
...
NMilitary = {
	...
	HARD_ATTACK_TARGETING_FACTOR = 1.0,		-- How much we care about potential hard attacks when evaluating priority combat target
	...
	HOURLY_ORG_MOVEMENT_IMPACT = 0.0,		-- how much org is lost every hour while moving an army.
	ZERO_ORG_MOVEMENT_MODIFIER = -0.8,		-- speed impact at 0 org.
	INFRA_ORG_IMPACT = 0.75,				-- scale factor of infra on org regain.
	...
	MAX_DIVISION_BRIGADE_HEIGHT = 3,		-- Max height of regiments in division designer.
	MIN_DIVISION_BRIGADE_HEIGHT = 2,		-- Min height of regiments in division designer.
	MAX_DIVISION_SUPPORT_WIDTH = 4,			-- Max width of support in division designer.
	MAX_DIVISION_SUPPORT_HEIGHT = 1,		-- Max height of support in division designer.

	BASE_DIVISION_BRIGADE_GROUP_COST = 5, 	--Base cost to unlock a regiment slot,
	...
	LAND_COMBAT_STR_ARMOR_DEFLECTION_FACTOR = 0.0, -- damage reduction if armor outclassing enemy
	LAND_COMBAT_ORG_ARMOR_DEFLECTION_FACTOR = 0.0, -- damage reduction if armor outclassing enemy
	...
	LAND_SPEED_MODIFIER = 0.4,                    -- basic speed control
	...
	OUT_OF_SUPPLY_ATTRITION = 0.40,                 -- max attrition when out of supply
	...
	FUEL_PENALTY_START_RATIO = 0.01,				-- ratio of fuel in an army to start getting penalties
	...
	ARMY_MAX_FUEL_FLOW_MULT = 0.0,					-- max fuel ratio that an army can get per hour, multiplied by supply situation

	ARMY_FUEL_COST_MULT = 0.0,						-- fuel cost multiplier for all army related stuff
	ARMY_COMBAT_FUEL_MULT =   0.0,					-- fuel consumption ratio in combat (plus ARMY_MOVEMENT_FUEL_MULT if you are also moving. ie offensive combat)
	ARMY_TRAINING_FUEL_MULT = 0.0,					-- fuel consumption ratio while training
	ARMY_MOVEMENT_FUEL_MULT = 0.0,					-- fuel consumption ratio while moving
	...
	FUEL_EFFICIENCY_RAID_MULTIPLIER = 0.0,			-- convoy raid multiplier for fuel sunk
...
NAir = {
	...
	THRUST_WEIGHT_AGILITY_FACTOR = 3,								-- For plane designs, additive Km/h max speed bonus per point of thrust exceeding weight
...
NNavy = {
	...
	NAVAL_SPEED_MODIFIER = 0.22,	                    				-- basic speed control
	NAVAL_RANGE_TO_INGAME_DISTANCE = 0.33,							-- Scale the ship stats "naval_range" to the ingame distance
	...
	IN_COMBAT_FUEL_COST = 0.0,										-- ships in combat will get this ratio for fuel cost
	...
	MAX_FUEL_FLOW_MULT = 0.0, -- max fuel flow ratio for ships, which will be multiplied by supply
	FUEL_COST_MULT = 0.0, -- fuel multiplier for all naval missions

	OUT_OF_FUEL_SPEED_FACTOR = 0,
	OUT_OF_FUEL_RANGE_FACTOR = 0,
	OUT_OF_FUEL_ATTACK_FACTOR = 0,
	OUT_OF_FUEL_TORPEDO_FACTOR = 0,
	...
	TRAINING_ORG = 0.5,												-- max organization on traiaing mission
...
NAI = {
	...
	DIPLOMACY_FACTION_GLOBAL_TENSION_FACTOR = 0.1,-- How much the AI takes global tension into account when considering faction actions
	...
	BASE_DISTANCE_TO_CARE = 2600.0,				-- Countries that are too far away are less interesting in diplomacy
	...
	UPDATE_SUPPLY_BOTTLENECKS_FREQUENCY_HOURS = 90;     -- Check for and try to fix supply bottlenecks this often. (168 hours = 1 week)
...
NSupply = {
	...
	NODE_INITIAL_SUPPLY_FLOW = 3.1,
	NODE_STARTING_PENALTY_PER_PROVINCE = 0.16,
	NODE_ADDED_PENALTY_PER_PROVINCE = 0.11,

	-- defines that are used for supply reach for dockyards
	NAVAL_BASE_INITIAL_SUPPLY_FLOW = 4.0,
	NAVAL_BASE_STARTING_PENALTY_PER_PROVINCE = 0.20,
	NAVAL_BASE_ADDED_PENALTY_PER_PROVINCE = 0.24,

	-- Node Flow (i.e. province caps) increase by this amount per railway level of the node's bottleneck
	NODE_FLOW_BONUS_PER_RAIL_LEVEL = 0.74,
	...
	AVAILABLE_MANPOWER_STATE_SUPPLY = 14.18,						--Factor for state supply from max manpower (population)
